...
def test_openclaw_plugin_manifest_matches_schema_example():
    ...
    assert package_json["version"] == "1.6.1"
    assert manifest["configSchema"] == schema
    assert schema["type"] == "object"
    assert schema["additionalProperties"] is False
    assert schema["properties"]["apiBaseUrl"]["default"] == "http://127.0.0.1:8766"
    ...
    assert "mcpCommand" not in schema["properties"]
    ...
    assert "node:child_process" not in (example_dir / "src" / "service.js").read_text(encoding="utf-8")
...
def test_openclaw_docs_match_install_contract():
    ...
    assert "--dangerously-force-unsafe-install" not in docs
