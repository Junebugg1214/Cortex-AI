...
`cortex scan` also auto-detects known local platform files and MCP config definitions from the compatibility matrix, so it can recognize tools you already have installed before Cortex has written anything itself. It stays read-only by default; use `cortex portable --from-detected ... --to all --project .` to explicitly adopt detected local context and sync it everywhere, or `cortex extract --from-detected ...` if you only want the graph.
